...
**注意**: この教訓は誤りでした。下記「2025-01-17 セッション」を参照。
...
**症状**: 会社計算規則を検索しても条文が表示されない（`paragraphs: 0`）

**直接的な原因**:
- `law_chunk_map.json` で会社計算規則がchunk 55にマッピングされていた
- 実際のR2データではchunk 56に存在

**根本原因の調査**:
全マッピングを検証した結果、**8,908件中ほぼ全件が不正確**だった:
- 8,003件: オフバイワン（map = R2データ - 1）
- 906件: その他の不整合
- 1件のみ正確

**なぜマッピングがズレていたか**:
1. `law_chunk_map.json`は`output_v2/laws_index_v2.json`（12/16作成）を基に生成されていた
2. R2のチャンクデータは12/26以降何度も更新された（sub_items対応など）
3. しかし`law_chunk_map.json`は**一度も更新されなかった**
4. `output_v2`と実際のR2データで法令の配置が異なっていた

**なぜ今まで動いていたか**:
1. **会社法・民法**: workerに`KAISHAHO_RANGES`と`MINPO_RANGES`がハードコードされており、`law_chunk_map`を使わない
2. **複数法令の同時検索**: 1つの法令が正しいチャンクを指していれば、同じチャンク内の他の法令も取得できた
3. **会社計算規則**: 単独で検索した場合にのみ問題が顕在化

**解決策**:
1. `r2_backup_20251226`から正しいマッピングを生成
   - スクリプト: `scripts/check_all_mapping.cjs`（検証用）
   - 出力: `correct_map_from_backup.json`
2. R2の`law_chunk_map.json`を正しいマップで上書き

**データソースの正しい理解**:
| ソース | 日付 | 内容 | 用途 |
|--------|------|------|------|
| `output_v2/` | 12/16 | 古いチャンクデータ | **使用禁止** |
| `r2_backup_20251226/` | 12/26 | R2の正確なバックアップ | **信頼できるソース** |
| `r2_subitems_fixed/` | 12/26〜 | sub_items追加版 | アップロード済み |

**12/30セッションの教訓の訂正**:
- ❌「`output_v2`が正しいデータ」→ ✅「`r2_backup_20251226`が正しいデータ」
- ❌「バックアップからのアップグレードは危険」→ ✅「`r2_backup_20251226`を基準にする」
- ❌「アップロード時はoutput_v2を基準にする」→ ✅「マッピングは`r2_backup_20251226`から生成する」
...
1. **マッピングを更新する際は必ず`r2_backup_20251226`を参照する**
